...
class levels(commands.Cog):
    ...
    @commands.command(aliases = ["dickslap", "ds", "cs"])
    async def cockslap(self, ctx, user: discord.Member = None):
        user_url = ctx.author.avatar_url if not user else user.avatar_url
        img = Image.open("slap.png")
        background = Image.open(requests.get(user_url, stream=True).raw)
        img = img.resize(background.size,Image.ANTIALIAS)
        background.paste(img, (0, 0), img)
        background.save('test.png',"PNG")
        await ctx.send(file=discord.File('test.png'))

    @commands.command(aliases=['jp'])
    async def jatinpainting(self, ctx, user: discord.Member = None):
        user_url = ctx.author.avatar_url if not user else user.avatar_url
        bg = Image.open("jatin.png").convert("RGBA")
        img = Image.open(requests.get(user_url, stream=True).raw).convert("RGBA").resize(bg.size)
        mask = Image.new("L", bg.size, 0)
        fake_mask = Image.open("jatinmask.png").resize(bg.size)
        mask.paste(fake_mask)
        im = Image.composite(img, bg, mask)
        im.save('nub.png',"PNG")
        await ctx.send(file=discord.File('nub.png'))

    @commands.command()
    async def sp(self, ctx, user: discord.Member = None):
        user_url = ctx.author.avatar_url if not user else user.avatar_url
        bg = Image.open("jatin.png").convert("RGBA")
        img = Image.open(requests.get(user_url, stream=True).raw).convert("RGBA").resize((423,331),Image.ANTIALIAS)
        new = Image.new("RGBA", bg.size)
        mask = Image.new("L", bg.size, 0)
        fake_mask = Image.open("jatinmask.png").resize(bg.size)
        mask.paste(fake_mask)
        
        new.paste(img, (15,300))
        new.putalpha(mask)
        new.save('n.png','PNG')
        await ctx.send(file=discord.File('n.png'))

        new_jatin = Image.open("jatininvis.png").convert("RGBA").resize(new.size)
        new.paste(new_jatin)
        new.save('b.png','PNG')
        await ctx.send(file=discord.File('b.png'))        
